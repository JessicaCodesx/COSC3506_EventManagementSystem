...
const InvoicesListPage: React.FC = () => {
  ...
  const filteredInvoices = invoices.filter(invoice => {
    const matchesSearch = 
      invoice.event.eventName.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="list-page-container">
            ...
            {loading ? (
                    ...
                    {filteredInvoices.map(invoice => (
                      <tr key={invoice.id}>
                        ...
                        <td className="actions-cell">
                          <Link to={`/invoices/${invoice.id}`} className="action-button" title="View Details">
                            <i className="fa fa-eye"></i>
                          </Link>
                          {(role === 'ADMIN' || (role === 'CLIENT' && invoice.status === 'PENDING')) && (
                            <Link to={`/invoices/${invoice.id}/edit`} className="action-button" title="Edit">
                              <i className="fa fa-edit"></i>
                            </Link>
                          )}
                          {role === 'CLIENT' && invoice.status === 'PENDING' && (
                            <Link to={`/invoices/${invoice.id}/pay`} className="action-button" title="Pay Now">
              ...
              <div className="empty-state">
                ...
                {(role === 'ADMIN' || role === 'CLIENT') && !searchTerm && statusFilter === 'ALL' && (
                  <Link to="/invoices/create" className="btn-primary">
                    Create Invoice
                  </Link>
                )}
